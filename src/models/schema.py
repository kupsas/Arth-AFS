...
class Asset(BaseModel, table=True):
    ...
    isin: Optional[str] = Field(max_length=12, default=None)
    category: AssetCategory = Field(index=True)
    sub_category: Optional[str] = Field(max_length=100, default=None)
    currency: str = Field(max_length=3, default="INR")
...
class Transaction(BaseModel, table=True):
    ...
    asset_id: Optional[int] = Field(foreign_key="assets.id", default=None)
    posted_at: datetime = Field(index=True)
    amount: Decimal = Field(max_digits=18, decimal_places=2)
    currency: str = Field(max_length=3, default="INR")
    txn_type: TransactionType = Field(index=True)
    raw_source_id: Optional[str] = Field(default=None)
